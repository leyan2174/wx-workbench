...
fn assert_account(web: &Web, name: &str) {
    ...
    let mut users: Vec<_> = all["contacts"].as_array().unwrap().iter()
        .map(|row| {
...
fn three_web_query_routes_use_real_daemon_v3_and_stay_account_bound() {
    ...
    for (web, name) in [(&first, "web-query-a"), (&second, "web-query-b"), (&first, "web-query-a")] {
        assert_account(web, name);
    }

    for path in ["/api/contacts", "/api/sessions", "/api/tag-members?name=Friends"] {
        let response = first.http.get(format!("{}{path}", first.origin))
            .header("x-wx-token", &second.token).send().unwrap();
        assert_eq!(response.status(), 401);
    }
    for path in ["/api/contacts?limit=0", "/api/sessions?limit=2001", "/api/tag-members?name=Friends&source=other"] {
        assert_eq!(first.request(reqwest::Method::GET, path, None, None).status(), 400);
    }

    // Web queries remain connect-only after an explicit daemon stop.
    let info = call(&fixture, &a, &["tasks", "info"]);
    let directory = fixture.root.join("shared-runtime/accounts")
        .join(info["runtime_id"].as_str().unwrap());
    success(fixture.run(&a, &["daemon", "stop"]));
    for path in ["/api/contacts", "/api/sessions", "/api/tag-members?name=Friends"] {
        assert_eq!(first.request(reqwest::Method::GET, path, None, None).status(), 503);
    }
