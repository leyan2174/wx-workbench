...
- 通用密钥流上限为 25 MiB，视频解码前缀仍为 128 KiB；预算见[WASM 契约](../../../src/toolkit/sns/VIDEO_RUNTIME.md)。
...
oracle.py 仅提取纯 URL 函数，不导入供应商模块或读取微信数据。普通回归不重建 golden。run.ps1 委托 tests/run-module.ps1，使用根 Cargo.toml 和 wx 测试目标，可显式指定 TargetDir，不再要求手工选择 rlib。环境与跳过规则见[测试说明](../../README.md)。
