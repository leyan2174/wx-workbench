...
wx-cli 以账号为隔离单位。CLI、MCP 和本地 Web 负责输入、输出与协议适配；daemon 装配账号能力、管理读取快照与执行生命周期，并按需要监督 worker。业务模型和规则按业务域独立，不因经过 daemon 就归执行宿主所有，也不意味着全部工作都运行在同一个进程或持久队列中。
...
当前输出收尾使用短期 `ExportContext` 复核固定账号，图片、单音频及离线视频向共享发布器提供保护清单。数据库快照允许写入配置指定的解密缓存，但保护原始库、配置、密钥和运行目录；普通导出还必须保护该缓存。主密钥获取器只返回验证后的材料，初始化执行宿主将主密钥和逐库密钥按观察到的变更序号一次原子保存。集中验证和失败后的定向复测已完成，实际命令、日志、独立夹具及未做真实账号验证的限制见[集中验收](architecture-final-validation.md)，不宣称首次全量命令无失败。
