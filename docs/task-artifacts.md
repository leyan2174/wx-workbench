...
媒体大小扫描只读取普通文件的元数据，不读取文件内容。扫描目录链需要目录列举权限：Windows 目录句柄允许共享读写但不共享删除，以阻止持有期间的目录改名或替换，同时允许子文件正常创建和发布。权限不足或身份变化按扫描失败状态报告，不降级为无保护扫描。目录固定不代表文件内容或统计大小被冻结，活动文件仍可能变化。
...
64 MiB 按完整渲染后的 UTF-8 字节数检查，超限文件不会发布；它不是渲染过程的内存峰值上限。当前实现会同时持有查询数据与渲染文本，大范围导出应结合查询响应预算选择时间范围和条数。
