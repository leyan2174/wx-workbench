...
| 分组 | 当前已有 | 尚未完成的跨入口范围 |
| --- | --- | --- |
| 导出与计划 | CLI 单聊/批量、delta、计划、表情、SNS 相册/快照/归档；目录任务支持 dry-run、媒体预算、结构化结果及三入口受控产物交付 | 顶层 export 的 Markdown/TXT/YAML、计划审阅/应用、增量更新及其他导出仍未完整跨入口公开。目录任务 JSON/CSV/HTML 不替代所有 CLI 导出；任务成功也不等于文件已被用户下载。 |
...
持久任务当前只注册六种公开 kind：wechat_keys、wechat_decrypt、image_key、export_all、decode_images、sns_decrypt。任务管理的 list/get/cancel/events 和受控 submit 是执行方式，不是所有业务的替代接口。MCP 任务默认关闭，宿主授权与模型请求不能互相替代。详见[任务服务](daemon-tasks.md)。
