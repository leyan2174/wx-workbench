...
本项目支持显式复用已保存账号材料、只读内存扫描和账号级捕获。它们是互斥授权路径；普通查询只使用已保存密钥，不自动启动扫描或重启微信。

## 选择来源

| 参数 | 行为 | 条件 |
| --- | --- | --- |
| `--key-provider saved` | 只使用统一 DPAPI 存储中的账号密钥并逐库验证；没有账号材料、损坏或验证失败时拒绝。 | 默认值，绝不降级到扫描或捕获。 |
...
检查路径与密钥提供器不会捕获密钥。`WX_CLI_CONFIG` 选择配置文件，`--db-dir` 选择数据库目录，二者不能混淆。现有配置及其密钥文件可使未指定 `--force` 的初始化直接返回。
...
`account_key.dpapi`、`all_keys.json` 和配置内图片材料不是受支持的读取来源；普通业务明确拒绝这些输入，不回退、不删除文件。`keys_file` 字段仅继续作为既有账号标识及正式存储绑定锚点，不用于读取旧材料。初始化、未验证标记和不支持文件的保留规则见[统一密钥存储](key-store.md)。

缓存命中前仍核对当前源库首页 HMAC。密钥失效或首页损坏时返回错误，不继续复用旧缓存，也不自动扫描或重启微信；先检查目标账号与密钥条件。

初始化拒绝畸形配置、旧图片密钥字段、冲突输出路径及检测到的并发修改，保留其他未知配置字段，并写入配置指定的加密存储。存储和配置分别原子发布，先材料后引用；这不是跨文件事务。配置发布失败时加密材料仍可能存在，核对选中账号后显式重试初始化。普通密钥轮换不重新发布未改变的配置文件。

`saved` 可复用保存的账号密钥为新增分片派生密钥，但仍需完整验证。记录缺失、不可读、目录不匹配或验证失败时保留原记录并明确报错，不静默扫描。首次初始化必须显式选择 `memory`，或选择同时带重启授权的 `account`。账号密钥不是登录密码，也不是永久跨设备身份；迁移或换钥后的数据库必须重新验证。
