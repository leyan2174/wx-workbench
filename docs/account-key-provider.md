...
旧 `account_key.dpapi`、`all_keys.json` 和配置图片材料的迁移入口及读取器已移除；普通业务明确拒绝旧输入，不回退、不删除旧文件。`keys_file` 字段仅继续作为既有账号标识及正式存储绑定锚点，不用于读取旧材料。初始化、未验证标记和旧文件保留规则见[统一密钥存储](key-store.md)。
