...
默认 `wx mcp` 注册 23 个查询及受控图片工具，见[查询协议](query-protocol.md)；后台任务默认关闭。任务类型仅覆盖共享能力列表中的业务子集，不代表所有导出、计划、媒体和治理能力均已跨入口接通。宿主在启动参数中授权，工具请求不能添加授权或改路径：
...
| `export_voices` | 原始 SILK 与关联证据导出 | 使用 `voice_export` 选集，只写新受控目录；MCP/Web 新提交需 `--task-allow-media-write`，MCP 读取另需产物读取授权；不转码或转写 |
| `chat_plan` | 生成不可变聊天计划 | 使用 `chat_plan` 请求；MCP/Web 的目录大小扫描另需 `--task-allow-plan-scan`，估算不隐式扫描 |
| `chat_plan_review` | 按引用修订计划选择标记 | 使用 `chat_plan_review` 请求和受控计划引用；MCP 除任务类型授权外还需 `--task-allow-artifact-read`；不接受任意计划文件路径 |
| `chat_plan_apply` | 执行计划选集并登记导出产物 | 使用 `chat_plan_apply` 请求和受控计划引用；MCP 除任务类型授权外还需 `--task-allow-artifact-read`；空选集不扩大为全量导出 |
...
- 运行任务在创建挂起进程时通过 Job 列表属性原子分配到 Windows Job，随后恢复执行；取消、正常退出和异常退出均回收 Job。不把已发布文件回滚为“从未执行”。
...
| 每任务日志 | 256 行；正文按最多 512 个 UTF-8 字节截断，保持字符完整，超长行另加 `[truncated]` 标记 |
...
幂等保证限于仍保留的历史记录，不承诺无限期 exactly-once。旧日志行被淘汰时可通过起始序号识别缺口；单行正文截断不改变该行序号。事件游标过期或重启后须按 `reset` 重新读取当前任务列表，不伪装为无遗漏连续日志。

日志脱敏按凭据特征和当前已知密钥隐藏命中的整行，不是聊天内容或本地路径的通用匿名化。日志与任务产物仍可能包含私人信息，公开问题报告前需要单独检查。
