...
pub fn cmd_init(
    ...
    // 只停止这份配置及该账号的旧后台；首次初始化不要求已存在运行上下文。
    let previous_runtime = crate::runtime::RuntimeContext::load()
        .ok()
        .filter(|runtime| {
            setup::same_path(&runtime.config_path, &config_path).unwrap_or(false)
                && setup::same_path(&runtime.config.db_dir, &db_dir).unwrap_or(false)
                && setup::same_path(&runtime.config.keys_file, &paths.keys_file).unwrap_or(false)
        });
    ...
    if let Some(runtime) = previous_runtime {
        if let Err(error) = crate::cli::transport::stop_runtime(&runtime) {
            eprintln!("原账号后台未停止：{error}");
        }
    }
