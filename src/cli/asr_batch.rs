...
use crate::{runtime::RuntimeContext, toolkit::asr::{batch::CacheOptions, local, local_python, openai, Backend}};
...
pub fn cmd(args: Args) -> Result<()> {
    ...
    ensure!(report.failed == 0, "{} voice messages failed; successful results were published", report.failed);
    ensure!(report.warnings.is_empty(), "ASR finished with persistence warnings; inspect report");
    Ok(())
}
...
fn configured_backend(runtime: &RuntimeContext, overrides: BackendArgs) -> Result<Backend> {
    let config: Value = serde_json::from_slice(&fs::read(&runtime.config_path)
        .context("read fixed-account transcription configuration")?)?;
    ensure!(config.is_object(), "transcription configuration must be an object");
    let configured = match config.get("transcription_backend") {
        None => "local",
        Some(value) => value.as_str().context("transcription_backend must be a string")?,
    };
    let backend = if matches!(overrides.backend, super::asr::BackendKind::ExplicitOpenAi) {
        "openai"
    } else { configured };
    let allow_upload = overrides.allow_upload;
    ensure!(overrides.timeout_seconds > 0, "ASR timeout must be positive");
    match backend {
        "local" => {
...
fn configured_api_key_with(
    ...
    let name = name.as_str().context("openai_api_key_env must be a string")?;
    ...
    ensure!(key.len() <= 16_384, "configured OpenAI credential exceeds limit");
    ensure!(!key.trim().is_empty(), "configured OpenAI credential environment variable is empty");
    Ok(key.trim().to_owned())
}

fn required_string<'a>(config: &'a Value, field: &str) -> Result<&'a str> {
    config.get(field).and_then(Value::as_str).filter(|text| !text.trim().is_empty())
...
fn discover_cpp_model() -> Result<PathBuf> {
    ...
    }).context("legacy model discovery requires USERPROFILE or HOMEPATH; specify --whisper-model")?;
    ...
    for name in ["whisper-models", "models", "Downloads"] {
        ...
        if let Some((name, path)) = candidates.into_iter().next() {
            ensure!(path.is_file(), "first legacy model candidate is not a regular file");
            eprintln!("[asr-batch] legacy model discovery selected {name}; use --whisper-model to pin explicitly");
            return path.canonicalize().context("resolve discovered whisper.cpp model");
        }
    }
...
mod credential_tests {
    ...
    fn absent_env_field_uses_inline_without_any_environment_lookup() {
        ...
        assert_eq!(configured_api_key_with(&config, |_| panic!("unexpected environment lookup")).unwrap(), SECRET);
        assert_redacted(configured_api_key_with(&json!({}), |_| panic!("unexpected environment lookup")).unwrap_err());
    }

    #[test]
    fn invalid_env_field_never_reads_environment_or_falls_back() {
        for name in [json!(null), json!(17), json!(true), json!([]), json!({}),
            json!(""), json!("9ASR"), json!("ASR-KEY"), json!("ASR=KEY"),
            json!(" ASR"), json!("ASR\n"), json!("ASR\0KEY"), json!("密钥"),
            json!("A".repeat(129)), json!(format!("{SECRET}-invalid"))] {
            let config = json!({"openai_api_key_env":name, "openai_api_key":SECRET});
            assert_redacted(configured_api_key_with(&config, |_| panic!("invalid name was read")).unwrap_err());
        }
    }
    ...
    fn env_name_length_boundary_is_accepted() {
        ...
        assert_eq!(configured_api_key_with(&config, |selected| {
    ...
    fn missing_or_non_unicode_env_does_not_fallback_or_leak() {
        ...
        for error in [std::env::VarError::NotPresent,
    ...
    fn empty_or_oversized_env_does_not_fallback_or_leak() {
        ...
        assert_eq!(configured_api_key_with(&config, |_| Ok(boundary.clone())).unwrap(), boundary);
    }
    ...
    fn cloud_runtime(root: &Path) -> RuntimeContext {
        ...
        RuntimeContext {
            config: crate::config::Config {
                db_dir: root.join("unused-db"), keys_file: root.join("unused-keys"),
                decrypted_dir: root.join("unused-decrypted"), wechat_process: String::new(),
            },
            config_path, root: root.to_owned(), id: "synthetic".into(), directory: root.join("unused-runtime"),
        }
    }
    ...
    fn cli_key_file_precedes_invalid_configured_env() {
        ...
        let backend = configured_backend(&runtime, BackendArgs {
            allow_upload: true, api_key_file: Some(key_file),
    ...
    fn upload_denial_precedes_cli_file_and_configured_env_access() {
        let root = tempfile::tempdir().unwrap();
        let runtime = cloud_runtime(root.path());
        let error = configured_backend(&runtime, BackendArgs {
            api_key_file: Some(root.path().join("missing-key.txt")), ..BackendArgs::default()
        }).unwrap_err();
