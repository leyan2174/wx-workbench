...
pub fn cmd(output: Option<String>, transcribe: bool, extra: Vec<String>) -> Result<()> {
    ...
    argv.extend(extra);
    let args = Args::try_parse_from(argv).unwrap_or_else(|error| error.exit());
    args.validate()?;
    let runtime = RuntimeContext::load()?;
    emit(export_for(&runtime, args)?)
}
