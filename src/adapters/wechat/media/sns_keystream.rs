...
impl std::fmt::Display for KeystreamError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::InvalidInput => "SNS media input exceeds limits or is empty",
            Self::AssetUnavailable => {
                "SNS keystream WASM is not distributed; provide an authorized local asset explicitly"
            }
            Self::AssetRead => "SNS keystream WASM asset could not be read",
            Self::UnsupportedAsset => "SNS keystream WASM asset does not match the supported ABI",
            Self::Runtime => "SNS keystream WASM failed or exhausted runtime limits",
            Self::InvalidMp4 => "SNS video restored prefix has no MP4 file type marker",
        })
    }
}
impl std::error::Error for KeystreamError {}
type Result<T> = std::result::Result<T, KeystreamError>;

/// 每次调用创建隔离 Store，密钥不进入 Debug、日志或持久缓存。
pub struct SnsKeystream {
    engine: Engine,
    module: Module,
...
fn failure(_: impl std::fmt::Display) -> KeystreamError {
    KeystreamError::Runtime
}
...
impl SnsKeystream {
    ...
    pub fn new(wasm_path: &Path, limits: RuntimeLimits) -> Result<Self> {
        ...
        File::open(wasm_path)
            .map_err(|_| KeystreamError::AssetRead)?
    ...
    /// 公开源码不分发授权不明的 WASM；生产调用必须显式传入已授权资产。
    ...
    pub fn bundled(limits: RuntimeLimits) -> Result<Self> {
        ...
        let asset = root.join("src/adapters/wechat/media/assets/wasm_video_decode.wasm");
        if !asset.is_file() {
            return Err(KeystreamError::AssetUnavailable);
        }
        Self::new(&asset, limits)
    }

    fn from_bytes(bytes: &[u8], limits: RuntimeLimits) -> Result<Self> {
        if bytes.len() > WASM_MAX_BYTES || Sha256::digest(bytes)[..] != WASM_SHA256 {
            return Err(KeystreamError::UnsupportedAsset);
        }
    ...
    pub fn keystream(&self, key: &str, size: usize) -> Result<Vec<u8>> {
        if key.len() > KEY_MAX_BYTES {
            return Err(KeystreamError::InvalidInput);
        }
        ...
        let aligned = size.checked_add(7).ok_or(KeystreamError::InvalidInput)? & !7;
        ...
        let outcome = (|| {
            ...
            let (ctor, raw) = store
                ...
                .binding
                .constructor
                .ok_or(KeystreamError::UnsupportedAsset)?;
            ...
            let ptr = instance
                ...
                .map_err(failure)?;
            if ptr <= 0 {
                return Err(KeystreamError::Runtime);
            }
            let p = ptr as usize;
    ...
    /// 恢复视频文件前缀，明文原样保留；ftyp 仅是格式标记，不是完整性认证。
    pub fn restore_video(&self, key: &str, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if !is_mp4(&output) {
            output.zeroize();
            return Err(KeystreamError::InvalidMp4);
        }
        Ok(output)
    }
}
