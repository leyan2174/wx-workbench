...
impl DatabaseMediaError {
    pub fn media_error(&self) -> crate::business::media::Error {
        use crate::business::media::{Error, Failure, Stage};
        let (stage, failure) = match self.kind {
            ErrorKind::InvalidIdentity => (Stage::Discovery, Failure::InvalidReference),
            ErrorKind::UnsafePath | ErrorKind::ActiveDatabase => {
                (Stage::Discovery, Failure::UnsafeSource)
            }
            ErrorKind::MissingDatabase => (Stage::Discovery, Failure::IncompleteSources),
            ErrorKind::UnsupportedSchema => (Stage::Discovery, Failure::Unsupported),
            ErrorKind::InvalidMessage | ErrorKind::NotVoice => {
                (Stage::Association, Failure::InvalidReference)
            }
            ErrorKind::MessageNotFound | ErrorKind::MediaNotFound => {
                (Stage::Association, Failure::NotFound)
            }
            ErrorKind::AmbiguousMessage
            | ErrorKind::AmbiguousContact
            | ErrorKind::AmbiguousMedia => (Stage::Association, Failure::Ambiguous),
            ErrorKind::ConflictingEvidence => (Stage::Association, Failure::ConflictingEvidence),
            ErrorKind::InvalidVoiceData => (Stage::Decode, Failure::InvalidMaterial),
            ErrorKind::LimitExceeded => (Stage::Discovery, Failure::LimitExceeded),
            ErrorKind::DatabaseRead => (Stage::Discovery, Failure::Unavailable),
            ErrorKind::SnapshotChanged => (Stage::Revalidation, Failure::StaleEvidence),
        };
...
pub fn resolve_voice_media_id(
    ...
    with_sources(sources, |opened, media| {
        ...
        let files: Vec<_> = opened
            .iter()
            .filter(|(source, _)| source.starts_with("message/message_"))
            .map(|(source, db)| SourceFile {
                logical_name: source.clone(),
                path: db.path.clone(),
                kind: SourceKind::Ordinary,
            })
            .collect();
        if files.is_empty() {
            return Err(error(ErrorKind::MissingDatabase, "message sources absent"));
        }
        let snapshot = Snapshot::open(files, [username.to_owned()])
            .map_err(|error| message_error(error, "legacy message snapshot"))?;
        ...
        let reference = snapshot
            .resolve_server_id(username, server, None, SourceKind::Ordinary)
            .map_err(|error| message_error(error, "legacy message association"))?;
        let message = snapshot
            .read_metadata(reference.evidence())
            .map_err(|error| message_error(error, "legacy message metadata"))?;
        if message
            .checked_server_id()
            .map_err(|_| error(ErrorKind::InvalidMessage, "server_id must be INTEGER"))?
            != Some(server)
        {
            return Err(error(
                ErrorKind::ConflictingEvidence,
        ...
        let source = message.logical_source.as_str();
        let db = opened
            .get(source)
            .ok_or_else(|| error(ErrorKind::MissingDatabase, "message source absent"))?;
        let id = message
            .local_id
            .ok_or_else(|| error(ErrorKind::InvalidMessage, "message local_id"))?;
        let time = message.timestamp;
        let kind = message.local_type;
        if id <= 0 {
            return Err(error(
        ...
        snapshot
            ...
            .map_err(|error| message_error(error, "legacy message revalidation"))?;
        Ok(voice)
    })
}

fn message_error(cause: anyhow::Error, stage: &'static str) -> DatabaseMediaError {
    use crate::adapters::wechat::messages::read::SchemaIssue;
    use crate::business::messages::Error as MessageError;
    if matches!(
        cause.downcast_ref::<SchemaIssue>(),
        Some(SchemaIssue::ShadowedSqliteRowid)
    ) {
        return error(ErrorKind::UnsupportedSchema, "shadowed SQLite rowid");
    }
    let kind = match cause.downcast_ref::<MessageError>() {
        Some(MessageError::NotFound) => ErrorKind::MessageNotFound,
        Some(MessageError::Ambiguous) => ErrorKind::AmbiguousMessage,
        Some(MessageError::Expired) => ErrorKind::SnapshotChanged,
        Some(MessageError::Unsupported) => ErrorKind::UnsupportedSchema,
        Some(MessageError::InvalidData) => ErrorKind::InvalidMessage,
        Some(MessageError::Limit) => ErrorKind::LimitExceeded,
        Some(MessageError::Unavailable) | None => ErrorKind::DatabaseRead,
    };
    error(kind, stage)
}
...
fn join_voice(
    ...
    use crate::adapters::wechat::messages::{Snapshot, SourceFile};
    use crate::business::messages::{MessageSelector, SourceKind};
    ...
    .map_err(|error| message_error(error, "message snapshot"))?;
    let streams = snapshot.streams_for(identity.username, SourceKind::Ordinary);
    if streams.is_empty()
        || streams.iter().any(|index| {
            let stream = &snapshot.streams()[*index];
            !stream.supports_identity() || !stream.supports_server_id()
        })
    {
        return Err(error(
            ErrorKind::UnsupportedSchema,
            "message identity schema",
        ));
    }
    let reference = snapshot
        .resolve(
        ...
        .map_err(|error| message_error(error, "message identity"))?;
    let raw = snapshot
        .read_metadata(reference.evidence())
        .map_err(|error| message_error(error, "message metadata"))?;
    if raw.logical_source != source || raw.local_id != Some(identity.local_id) {
        return Err(error(
            ErrorKind::ConflictingEvidence,
            "message identity mismatch",
        ));
    }
    if raw.local_type & 0xffff_ffff != 34 {
        return Err(error(ErrorKind::NotVoice, "message type"));
    }
    let create_time = raw.timestamp;
    if matches!(
        raw.server_id,
        crate::adapters::wechat::messages::StoredScalar::Integer(0)
    ) {
        return Err(error(
            ErrorKind::ConflictingEvidence,
    ...
    let server_id = raw
        .checked_server_id()
        .map_err(|_| error(ErrorKind::InvalidMessage, "server_id must be INTEGER"))?
        .ok_or_else(|| error(ErrorKind::InvalidMessage, "server_id must be INTEGER"))?;
    let tables = snapshot
        .streams_for(identity.username, SourceKind::Ordinary)
        .into_iter()
        .map(|stream| snapshot.streams()[stream].table_name().to_owned());
    let table = crate::business::messages::unique(tables)
        .map_err(|error| message_error(error.into(), "message table evidence"))?;
    ...
    for (media_source, db) in media {
        ...
        if let Some(chat_id) = chat_id {
                ...
                .map_err(|_| error(ErrorKind::DatabaseRead, "media row"))?
            {
                ...
                crate::business::media::verify_voice_join(
                    server_id,
                    create_time,
                    chat_id,
                    proven_server,
    ...
    snapshot
        .revalidate(&reference)
        .map_err(|error| message_error(error, "message revalidation"))?;
...
mod outcome_tests {
    ...
    #[test]
    fn source_and_evidence_failures_never_become_absence() {
        for (kind, expected) in [
            (ErrorKind::MissingDatabase, Failure::IncompleteSources),
            (ErrorKind::UnsupportedSchema, Failure::Unsupported),
            (ErrorKind::AmbiguousMessage, Failure::Ambiguous),
            (ErrorKind::AmbiguousMedia, Failure::Ambiguous),
            (ErrorKind::ConflictingEvidence, Failure::ConflictingEvidence),
            (ErrorKind::SnapshotChanged, Failure::StaleEvidence),
        ] {
            let classified = error(kind, "synthetic-private-source").media_error();
            assert_eq!(classified.failure, expected);
            assert_ne!(classified.failure, Failure::NotFound);
            assert!(!classified.to_string().contains("synthetic-private-source"));
        }
        assert_eq!(
            error(ErrorKind::SnapshotChanged, "changed")
                .media_error()
                .stage,
            Stage::Revalidation
        );
        assert_eq!(
            error(ErrorKind::MediaNotFound, "absent")
                .media_error()
                .failure,
            Failure::NotFound
        );
