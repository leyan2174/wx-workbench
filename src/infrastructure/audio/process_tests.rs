...
pub(crate) fn fake_ffmpeg() -> &'static Path {
...
fn run_failure(mode: &str, cancel: bool) {
    ...
    let timeout = if mode == "flood" {
        Duration::from_secs(5)
    } else {
        Duration::from_secs(1)
    };
    let result = convert_controlled(&source, &output, &executable, start + timeout, || {
    ...
    assert!(start.elapsed() < timeout + Duration::from_secs(4));
