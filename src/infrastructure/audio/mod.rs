...
const MAX_PREPARED_AUDIO_BYTES: usize = 32 * 1024 * 1024;
...
pub fn prepare_wav_bytes(bytes: &[u8]) -> Result<Vec<u8>> {
        ...
        bytes.len() <= MAX_PREPARED_AUDIO_BYTES,
        "input exceeds {MAX_PREPARED_AUDIO_BYTES} byte limit"
    );
    ...
    if bytes.starts_with(b"#!SILK_V3") || bytes.starts_with(b"\x02#!SILK_V3") {
        return pcm24k_to_wav(&decode_silk_to_pcm(bytes)?);
    }
...
pub fn pcm24k_to_wav(pcm: &[u8]) -> Result<Vec<u8>> {
        ...
        pcm.len() <= MAX_PREPARED_AUDIO_BYTES - 44,
...
pub(crate) fn convert_controlled(
    input: &Path,
    output: &Path,
    ffmpeg: &Path,
...
fn convert_controlled_checked(
    ...
    let publication = target.write_with_checked(
        |encoded| {
            ...
            crate::private_file::restrict(pcm_file.as_file())?;
...
pub(crate) fn publish_encoded(
    staged: &Path,
    target: crate::infrastructure::publication::ExportTarget,
