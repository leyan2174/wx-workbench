...
pub(crate) fn metadata_only_file(path: &Path) -> Result<(u32, u64, u64)> {
...
impl PinnedFile {
    pub(crate) fn open(path: &Path, deletable: bool) -> Result<Self> {
    ...
    pub(crate) fn snapshot(&mut self) -> Result<Fingerprint> {
        self.guard.verify()?;
    ...
    pub(crate) fn json_bytes(&mut self) -> Result<zeroize::Zeroizing<Vec<u8>>> {
        let before = fingerprint(information(&self.file, false)?, String::new());
    ...
    pub(crate) fn prefix(&mut self) -> Result<[u8; 16]> {
        self.file.seek(SeekFrom::Start(0))?;
    ...
    pub(crate) fn directories(&self) -> Result<Vec<DirectoryIdentity>> {
    ...
    pub(crate) fn verify_directories(&self, expected: &[DirectoryIdentity]) -> Result<()> {
        ensure!(self.directories()? == expected, "文件祖先目录身份变化");
        Ok(())
    }

    /// 不按路径重新打开删除目标；处置标志仅作用于已校验并独占写/删除的句柄。
    pub(crate) fn delete(mut self, expected: &Fingerprint) -> Result<()> {
...
pub(crate) fn runtime_locks(directory: &Path) -> Result<(HostOutputGuard, Vec<File>)> {
