...
impl LocalPythonConfig {
    pub fn discover(
        ...
        let python = discover_python(&working_root).map_err(|error| format!("{error:#}"));
        let environment = [
            "PATH",
            "SYSTEMROOT",
            "WINDIR",
    ...
    pub(crate) fn cache_identity(&self) -> Result<String> {
...
fn discover_python(config_parent: &Path) -> Result<PathBuf> {
    let mut candidates = Vec::new();
    if let Some(environment) = std::env::var_os("VIRTUAL_ENV") {
        candidates.push(PathBuf::from(environment).join("Scripts/python.exe"));
    }
    candidates.push(config_parent.join(".venv/Scripts/python.exe"));
    if let Ok(executable) = std::env::current_exe() {
        if let Some(directory) = executable.parent() {
            candidates.push(directory.join(".venv/Scripts/python.exe"));
        }
    }
    for candidate in candidates {
        if candidate.is_file() {
            return candidate
                .canonicalize()
                .context("resolve Python interpreter");
        }
    }
    resolve_python(PathBuf::from("python"))
}

fn resolve_python(path: PathBuf) -> Result<PathBuf> {
    if path.is_file() {
        return path.canonicalize().context("resolve Python interpreter");
    }
    ensure!(
        path.components().count() == 1,
        "configured Python interpreter is unavailable"
    );
    let search = std::env::var_os("PATH").context("Python interpreter not found")?;
    ...
    bail!("Python interpreter unavailable; activate a virtual environment or configure PATH")
}
...
pub fn transcribe_wav(config: &LocalPythonConfig, wav: &[u8]) -> Result<Transcription> {
    ...
    crate::infrastructure::audio::validate_wav(wav)?;
...
impl Worker {
    fn start(config: &LocalPythonConfig) -> Result<Self> {
        ...
        let (program_hash, program) = super::file_digest(python)?;
        let mut files = vec![program];
        let model_file = config.working_root.join(&config.model);
        if model_file.is_file() {
            let (_, file) = super::file_digest(&model_file)?;
            files.push(file);
        }
...
mod root_tests {
    ...
    fn packaged_config_parent_needs_no_legacy_scripts() {
        let directory = tempfile::tempdir().unwrap();
        let root = inference_root(directory.path()).unwrap();
    ...
    fn worker_without_legacy_scripts_reaches_executable_validation() {
        ...
        let config = LocalPythonConfig {
            ...
            working_root: inference_root(directory.path()).unwrap(),
            model: "synthetic.pt".into(),
            language: None,
            threads: None,
    ...
    fn configuration_root_must_be_absolute_existing_directory() {
        assert!(inference_root(Path::new("relative-config")).is_err());
        let directory = tempfile::tempdir().unwrap();
        assert!(inference_root(&directory.path().join("missing")).is_err());
        let file = directory.path().join("not-directory");
        fs::write(&file, b"synthetic").unwrap();
        assert!(inference_root(&file).is_err());
    }
}
