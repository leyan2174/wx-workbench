...
use crate::infrastructure::publication::{
    check_target, exists, inspect, parent_guard, resolve, same_path,
};
...
impl Snapshot {
    ...
    pub(crate) fn write_bytes(&self, bytes: &[u8], protected: &[PathBuf]) -> Result<()> {
        ...
        crate::private_file::restrict(temporary.as_file())?;
...
impl ConfigDocument {
    pub fn load(path: &Path) -> Result<Self> {
        ...
        crate::config::validate_key_configuration(&value)?;
        for key in [
            "db_dir",
            "keys_file",
            "key_store",
...
mod tests {
    #[test]
    fn inline_legacy_configuration_is_rejected_without_rewriting_secrets() {
        let root = tempfile::tempdir().unwrap();
        let db = root.path().join("account/db_storage");
        std::fs::create_dir_all(&db).unwrap();
        let path = root.path().join("config.json");
        std::fs::write(&path, br#"{"image_xor_key":136}"#).unwrap();
        let error = super::ConfigDocument::load(&path).err().unwrap();
        assert!(error.to_string().contains("Legacy inline image keys"));
        assert_eq!(std::fs::read(&path).unwrap(), br#"{"image_xor_key":136}"#);
    ...
    #[test]
    fn bound_account_initializes_current_store_without_reading_old_files() {
        let root = tempfile::tempdir().unwrap();
        let db = root.path().join("account/db_storage");
        std::fs::create_dir_all(&db).unwrap();
        let path = root.path().join("config.json");
        ...
        for name in ["all_keys.json", "account_key.dpapi"] {
            std::fs::write(root.path().join(name), b"deliberately invalid old material").unwrap();
        }
        let document = ConfigDocument::load(&path).unwrap();
        let configured = document.with_db(&db).unwrap();
        assert_eq!(configured["key_store"], "keys.dpapi");
        assert_eq!(configured["unrelated_setting"]["preserve"], true);
        assert_eq!(std::fs::read(&path).unwrap(), original);
        for name in ["all_keys.json", "account_key.dpapi"] {
            assert_eq!(
                std::fs::read(root.path().join(name)).unwrap(),
                b"deliberately invalid old material"
            );
        }
        assert!(!root.path().join("keys.dpapi").exists());
    }
