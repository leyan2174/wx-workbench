...
pub(crate) fn inspect(
    ...
    PathBuf,
    crate::attachment::local_files::HostOutputGuard,
    bool,
)> {
...
pub(crate) fn parent_guard(path: &Path) -> Result<crate::attachment::local_files::HostOutputGuard> {
    use crate::attachment::local_files::HostOutputGuard;
    local_path(path)?;
    ...
    for part in missing.components() {
        ...
        match fs::create_dir(&current) {
            ...
            Err(error) => return Err(error).context("创建配置父目录失败"),
        }
...
pub(crate) fn collect(root: &Path, extension: &str, skip_migrate: bool) -> Result<Vec<PathBuf>> {
    fn walk(
        base: &Path,
        root: &Path,
        ext: &str,
...
pub(crate) fn decryption_protected(runtime: &crate::runtime::RuntimeContext) -> Vec<PathBuf> {
    let mut paths = vec![
...
fn publish_checked(
    ...
    crate::private_file::restrict(&completed)?;
    completed.sync_all()?;
    drop(completed);
...
mod export_tests {
    ...
    fn protects_existing_future_and_ancestor_account_paths_without_writing() {
        ...
        crate::private_file::assert_private_acl(&output);
    }
    ...
    fn explicit_protection_and_checked_overwrite_share_publication_policy() {
        ...
        assert!(ExportTarget::capture_paths(&alias, std::slice::from_ref(&protected)).is_err());
        fs::remove_file(&alias).unwrap();
        let output = dir.path().join("result.bin");
        fs::write(&output, b"old").unwrap();
        ExportTarget::capture_paths(&output, std::slice::from_ref(&protected))
            .unwrap()
