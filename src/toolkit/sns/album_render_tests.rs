...
#[test]
fn python_ast_golden() {
    let output = std::process::Command::new("python")
        .arg(fixtures().join("oracle.py"))
        .output()
        .unwrap();
    assert!(
    ...
    let golden: Value = serde_json::from_slice(&output.stdout).unwrap();
...
fn preview_is_current() {
    ...
    // 仅测试入口显式生成验收 HTML，生产 render 不接触文件系统。
    std::fs::write(fixtures().join("preview.html"), result.html).unwrap();
}
