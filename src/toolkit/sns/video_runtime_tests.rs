...
fn asset() -> std::path::PathBuf {
    ...
    let root = if manifest.join("vendor").is_dir() {
        manifest
    } else {
        manifest.join("../../..")
    };
    root.join("vendor/wechat-decrypt/sns_media_wasm/wasm_video_decode.wasm")
}

#[test]
fn node_vectors_byte_exact() {
    let runtime = VideoRuntime::new(&asset(), RuntimeLimits::default()).unwrap();
...
fn decode_prefix_and_plaintext() {
    ...
    for size in [
        ...
        assert_eq!(runtime.decode("42", &encrypted).unwrap(), plain);
        assert_eq!(runtime.decode("", &plain).unwrap(), plain);
    }
    assert_eq!(
        runtime.decode("42", &[0; 15]),
        Err(VideoRuntimeError::InvalidMp4)
    );
}
...
fn rejects_inputs_and_resource_exhaustion() {
    ...
    for (key, size) in [
        ...
        assert_eq!(
            ...
            Err(VideoRuntimeError::InvalidInput)
        );
    }
    assert_eq!(
    ...
    for limits in [
        ...
        let bounded = VideoRuntime::new(&asset(), limits).unwrap();
        assert_eq!(bounded.keystream("1", 8), Err(VideoRuntimeError::Runtime));
    }
    let bounded = VideoRuntime::new(
        &asset(),
        RuntimeLimits {
            input_bytes: 4,
            ..RuntimeLimits::default()
    ...
    .unwrap();
    assert_eq!(
        bounded.decode("1", &[0; 5]),
        Err(VideoRuntimeError::InvalidInput)
    );
    assert_eq!(
        bounded.keystream("1", 5),
        Err(VideoRuntimeError::InvalidInput)
    );
}
...
fn large_node_oracles_and_finite_default_fuel() {
    ...
    for v in vectors.as_array().unwrap() {
        ...
        if size > MAX_KEYSTREAM_BYTES {
            ...
            assert_eq!(actual, Err(VideoRuntimeError::InvalidInput));
        } else {
...
fn fuel_budget_is_bounded_and_respects_caller() {
    ...
    for limits in [
        ...
        let runtime = VideoRuntime::bundled(limits).unwrap();
        assert_eq!(
            runtime.keystream("42", MAX_KEYSTREAM_BYTES),
            Err(VideoRuntimeError::Runtime)
        );
    }
    let runtime = VideoRuntime::bundled(RuntimeLimits {
        fuel: BASE_FUEL,
...
fn video_still_decrypts_only_128k_prefix() {
    ...
    assert_eq!(runtime.decode("", &large_plain).unwrap(), large_plain);
}
...
fn rejects_unknown_or_missing_assets() {
    assert!(matches!(
        ...
        Err(VideoRuntimeError::AssetRead)
    ));
    let text_asset = asset().with_file_name("wasm_video_decode.js");
    assert!(matches!(
        VideoRuntime::new(&text_asset, RuntimeLimits::default()),
        Err(VideoRuntimeError::UnsupportedAsset)
    ));
}
...
fn invalid_key_is_redacted_and_store_is_isolated() {
    ...
    assert_eq!(error, VideoRuntimeError::Runtime);
