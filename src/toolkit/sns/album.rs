...
use super::{album_images, album_render, album_videos, video_runtime::VideoRuntime};
...
type Engine = OnceLock<Option<VideoRuntime>>;
...
fn image_job(
    ...
    for (url, key, token) in [
        ...
        let outcome = album_images::download_sns_image(
            ...
            || {
                engine
                    .get_or_init(|| VideoRuntime::bundled(Default::default()).ok())
                    .as_ref()
