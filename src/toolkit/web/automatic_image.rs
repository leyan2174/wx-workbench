...
use super::{preview, query, server_types::Shared};
...
fn identity(encoded: &str) -> Result<AttachmentId> {
    let id = preview::identity(encoded)?;
...
pub async fn decode(
    ...
    let permit = DECODES.try_acquire().map_err(|_| Failure::Busy)?;
...
pub async fn drain() -> bool {
    tokio::time::timeout(Duration::from_secs(30), DECODES.acquire_many(2))
        .await
...
mod tests {
    ...
    async fn shared_query_saturation_returns_busy_and_preserves_other_failures() -> Result<()> {
        use super::super::server_types::{Records, Settings};
        use std::sync::Mutex;
        ...
        let _held = state.queries.acquire().await?;
    ...
    #[test]
    fn source_query_requires_only_source() {
        let query: SourceQuery =
            serde_json::from_value(json!({"source":"message/message_0.db"})).unwrap();
        assert_eq!(query.source, "message/message_0.db");
        for invalid in [
            json!({}),
            json!({"source":null}),
            json!({"source":7}),
            json!({"source":"message/message_0.db","output_root":"elsewhere"}),
            json!({"source":"message/message_0.db","image_key_file":"elsewhere"}),
        ] {
            assert!(serde_json::from_value::<SourceQuery>(invalid).is_err());
        }
    }
