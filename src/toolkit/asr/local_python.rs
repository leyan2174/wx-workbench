...
impl LocalPythonConfig {
    pub fn discover(model: String, language: Option<String>, threads: Option<usize>,
        timeout: Duration, temp_root: PathBuf, config_parent: PathBuf) -> Result<Self> {
        ensure!(!model.trim().is_empty() && model.len() <= 4096 && !model.chars().any(char::is_control),
        ...
        ensure!(!timeout.is_zero() && threads.is_none_or(|n| n > 0), "invalid local inference limits");
        if let Some(language) = &language {
            ensure!(!language.is_empty() && language.bytes().all(|c| c.is_ascii_alphabetic() || c == b'-'),
        ...
        let environment = ["PATH", "SYSTEMROOT", "WINDIR", "USERPROFILE", "HOMEDRIVE", "HOMEPATH",
            "APPDATA", "LOCALAPPDATA", "PROGRAMDATA", "PROGRAMFILES", "PROGRAMFILES(X86)",
            "COMMONPROGRAMFILES", "XDG_CACHE_HOME", "TORCH_HOME", "CUDA_VISIBLE_DEVICES",
            "CUDA_DEVICE_ORDER", "CUDA_MODULE_LOADING", "OMP_NUM_THREADS", "MKL_NUM_THREADS",
            "NUMBA_NUM_THREADS", "KMP_DUPLICATE_LIB_OK", "CUDA_PATH", "HTTP_PROXY", "HTTPS_PROXY",
            "ALL_PROXY", "NO_PROXY", "SSL_CERT_FILE", "SSL_CERT_DIR", "PYTHONPATH",
            "PYTHONHOME", "PYTHONUSERBASE", "PYTHONNOUSERSITE", "PYTHONSAFEPATH", "VIRTUAL_ENV",
            "CONDA_PREFIX", "CONDA_DLL_SEARCH_MODIFICATION_ENABLE"]
            .into_iter().filter_map(|name| std::env::var_os(name).map(|value| (name.into(), value))).collect();
        Ok(Self { timeout, deadline: None, python, working_root, model, language, threads, temp_root,
            environment, worker: Arc::new(Mutex::new(None)) })
    }
    ...
    pub(crate) fn host_input_paths(&self) -> Vec<PathBuf> {
        ...
        if model.is_file() { paths.push(model); }
        paths
    }
    ...
    pub(crate) fn tighten_timeout(&mut self, remaining: Duration) -> Result<()> {
        ...
        let deadline = Instant::now().checked_add(remaining).context("invalid inference deadline")?;
    ...
    fn remaining_timeout(&self) -> Result<Duration> {
        let remaining = self.deadline.map_or(self.timeout,
            |deadline| self.timeout.min(deadline.saturating_duration_since(Instant::now())));
        ensure!(!remaining.is_zero(), "local inference deadline expired");
        Ok(remaining)
    }
    ...
    fn with_worker<T>(&self, operation: impl FnOnce(&mut Worker) -> Result<T>) -> Result<T> {
        ...
        let mut slot = loop {
            ...
            match self.worker.try_lock() {
                Ok(slot) => break slot,
                Err(std::sync::TryLockError::Poisoned(_)) => bail!("local inference worker lock poisoned"),
                Err(std::sync::TryLockError::WouldBlock) => {
                    ensure!(start.elapsed() < self.timeout, "local inference worker busy timeout");
        ...
        if let Some(worker) = slot.as_mut() {
            if worker.process.child.try_wait()?.is_some() {
                if let Some(mut worker) = slot.take() { worker.shutdown()?; }
            }
        }
        if slot.is_none() { *slot = Some(Worker::start(self)?); }
        let result = self.remaining_timeout().and_then(|_| operation(slot.as_mut().expect("worker started")));
        if result.is_err() {
            if let Some(mut worker) = slot.take() {
                worker.shutdown().context("local inference failed and cleanup failed")?;
            }
        }
        result
    }
...
fn inference_root_with(config_parent: &Path, explicit_root: Option<&std::ffi::OsStr>) -> Result<PathBuf> {
    ensure!(config_parent.is_absolute(), "inference requires an absolute configuration directory");
    let config_parent = config_parent.canonicalize().context("resolve selected configuration directory")?;
    ensure!(config_parent.is_dir(), "selected configuration parent must be a directory");
    ...
    if let Some(root) = explicit_root.filter(|root| !root.is_empty()) {
        ...
        if root.is_absolute() && root.is_dir() {
            return root.canonicalize().context("resolve explicit inference working directory");
        }
    }
    Ok(config_parent)
}
...
mod root_tests {
    ...
    fn synthetic_config(root: &Path) -> LocalPythonConfig {
        LocalPythonConfig {
            timeout: Duration::from_secs(120), deadline: None,
            python: Err("SYNTHETIC_PRIVATE_PYTHON_PATH".into()), working_root: root.to_owned(),
            model: "synthetic.pt".into(), language: None, threads: None,
            temp_root: root.join("private-work"), environment: Vec::new(),
    ...
    fn worker_without_legacy_scripts_reaches_executable_validation() {
        ...
        let config = LocalPythonConfig {
            ...
            model: "synthetic.pt".into(), language: None, threads: None,
            temp_root: directory.path().join("private-work"), environment: Vec::new(),
            worker: Arc::new(Mutex::new(None)),
        };
    ...
    fn explicit_relative_legacy_root_and_model_use_config_parent() {
        ...
        assert_eq!(root.join("weights").join("synthetic.pt"), legacy.canonicalize().unwrap().join("weights").join("synthetic.pt"));
        assert!(!root.join("mcp_server.py").exists());
    }
    ...
    fn explicit_absolute_root_is_preserved_and_unavailable_root_falls_back() {
        ...
        assert_eq!(inference_root_with(directory.path(), Some(legacy.path().as_os_str())).unwrap(), legacy.path().canonicalize().unwrap());
        for explicit in [OsStr::new(""), OsStr::new("missing-legacy-root")] {
            assert_eq!(inference_root_with(directory.path(), Some(explicit)).unwrap(), directory.path().canonicalize().unwrap());
        }
    }
...
fn resolve_python(path: PathBuf) -> Result<PathBuf> {
    if path.is_file() { return path.canonicalize().context("resolve legacy Python"); }
    ensure!(path.components().count() == 1, "configured legacy Python is unavailable");
    ...
    for directory in std::env::split_paths(&search) {
        for name in [path.clone(), path.with_extension("exe")] {
            ...
            if candidate.is_file() { return candidate.canonicalize().context("resolve legacy Python"); }
        }
    }
...
pub fn transcribe_wav(config: &LocalPythonConfig, wav: &[u8]) -> Result<Transcription> {
    ensure!(wav.len() <= super::MAX_AUDIO_BYTES, "local inference WAV exceeds limit");
    ...
    config.with_worker(|worker| {
        ...
        worker.clear_response()?;
        Ok(Transcription {
            text: result.get("text").and_then(Value::as_str).context("inference text missing")?.to_owned(),
            language: result.get("language").and_then(Value::as_str).context("inference language missing")?.to_owned(),
...
impl Worker {
    fn start(config: &LocalPythonConfig) -> Result<Self> {
        ...
        let python = config.python.as_ref().map_err(|error| anyhow::anyhow!("{error}"))?;
        ensure!(config.working_root.is_dir(), "fixed inference working directory unavailable");
        fs::create_dir_all(&config.temp_root).context("create account inference temporary root")?;
        let work = tempfile::Builder::new().prefix("local-inference-").tempdir_in(&config.temp_root)?;
        ...
        ensure!(settings.len() <= 8192, "inference configuration limit exceeded");
        command.arg("-S").arg("-u").arg("-B").arg("-c").arg(BRIDGE)
            .arg(std::str::from_utf8(&settings)?)
            .current_dir(&config.working_root).env_clear().envs(config.environment.iter().cloned())
            .env("PYTHONUTF8", "1").env("PYTHONDONTWRITEBYTECODE", "1")
            .env("TMP", work.path()).env("TEMP", work.path())
        ...
        config.remaining_timeout()?;
        let mut process = Process { child: command.spawn().context("start legacy local inference")?,
            #[cfg(windows)] job: None };
        #[cfg(windows)] { process.job = Some(supervision::Job::attach(&process.child, Caller::Python)?); }
        let stdout = process.child.stdout.take().context("inference stdout unavailable")?;
        let stderr = process.child.stderr.take().context("inference stderr unavailable")?;
        let mut worker = Self { process, stdout, stderr, work: Some(work), identity: String::new(), _identity_files: files };
        // 空管道中仅写一个握手字节；之后请求用原子文件发布，避免阻塞管道写绕过超时。
        worker.process.child.stdin.as_mut().context("inference gate unavailable")?.write_all(b"G")?;
        worker.process.child.stdin.take();
        let ready = worker.response(config.remaining_timeout()?)?;
        let engine = ready.get("engine_identity").and_then(Value::as_str).context("inference engine identity missing")?;
        ensure!(engine.len() == 64 && engine.bytes().all(|b| b.is_ascii_hexdigit()), "invalid inference engine identity");
        let identity = serde_json::to_vec(&("legacy-python-local-v1", program_hash, engine,
            python.to_string_lossy(), config.working_root.to_string_lossy(), BRIDGE))?;
        worker.identity = format!("{:x}", Sha256::digest(identity));
        Ok(worker)
    }

    fn root(&self) -> &Path { self.work.as_ref().expect("worker directory alive").path() }
    ...
    fn send(&mut self, request: &Value) -> Result<()> {
        ...
        fs::rename(temporary, self.root().join("request.json")).context("publish local inference request")
    }
    ...
    fn response(&mut self, timeout: Duration) -> Result<Value> {
        ...
        loop {
            ensure!(start.elapsed() < timeout, "legacy local inference timed out");
            supervision::drain(&mut self.stdout, &mut stream_bytes, MAX_STREAM, Caller::Python)?;
            supervision::drain(&mut self.stderr, &mut stream_bytes, MAX_STREAM, Caller::Python)?;
            check_disk(self.root(), start, timeout)?;
            let path = self.root().join("response.json");
            if path.is_file() {
                let mut bytes = Vec::new();
                File::open(path)?.take(MAX_RESPONSE + 1).read_to_end(&mut bytes)?;
                ensure!(bytes.len() as u64 <= MAX_RESPONSE, "local inference response limit exceeded");
                let response: Value = serde_json::from_slice(&bytes)
                    .map_err(|_| anyhow::anyhow!("invalid local inference response; output withheld"))?;
                if response.get("ok").and_then(Value::as_bool) != Some(true) {
                    let category = match response.get("error").and_then(Value::as_str) {
                        Some("dependency_or_model_configuration") => "dependency_or_model_configuration",
            ...
            ensure!(self.process.child.try_wait()?.is_none(), "legacy local inference exited; process output withheld");
        ...
        if let Some(work) = self.work.as_ref() {
            ...
            loop {
                match fs::remove_dir_all(work.path()) {
                    ...
                    Err(_) if start.elapsed() < Duration::from_secs(1) => thread::sleep(Duration::from_millis(10)),
...
impl Drop for Worker {
    fn drop(&mut self) {
        if self.shutdown().is_err() { eprintln!("[asr-batch] legacy local inference cleanup failed"); }
    }
}

struct Process {
    child: Child,
    #[cfg(windows)] job: Option<supervision::Job>,
}
...
impl Process {
    fn stop(&mut self) -> Result<()> {
        ...
        while self.child.try_wait()?.is_none() {
            ensure!(start.elapsed() < Duration::from_secs(2), "local inference reap timed out");
            thread::sleep(Duration::from_millis(5));
        }
        #[cfg(windows)]
        job_result?;
...
fn check_disk(root: &Path, start: Instant, timeout: Duration) -> Result<()> {
    ...
    while let Some(directory) = pending.pop() {
        for entry in fs::read_dir(directory)? {
            ...
            count += 1;
            ensure!(count <= MAX_ENTRIES, "local inference temporary entry limit exceeded");
            let entry = entry?;
            let metadata = fs::symlink_metadata(entry.path())?;
            ensure!(!metadata.file_type().is_symlink(), "local inference temporary link rejected");
            #[cfg(windows)] {
                use std::os::windows::fs::MetadataExt;
                ensure!(metadata.file_attributes() & 0x400 == 0, "local inference reparse point rejected");
            }
            if metadata.is_dir() { pending.push(entry.path()); }
            else {
                ensure!(metadata.is_file(), "local inference temporary special file rejected");
                bytes = bytes.saturating_add(metadata.len());
                ensure!(bytes <= MAX_TEMP, "local inference temporary byte limit exceeded");
                if entry.file_name() == "response.json" || entry.file_name() == "response.tmp" {
                    ensure!(metadata.len() <= MAX_RESPONSE, "local inference response limit exceeded");
                }
            }
        }
    }
