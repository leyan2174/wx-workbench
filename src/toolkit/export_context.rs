...
pub(crate) struct ExportContext(Configuration);
...
impl ExportContext {
    ...
    pub(crate) fn protected(&self, input: &Path) -> Result<Vec<PathBuf>> {
        let mut paths = match &self.0 {
            Configuration::Present { runtime, .. } => super::files::export_protected(runtime),
...
mod tests {
    ...
    fn expected_account_rejects_missing_or_changed_config_without_environment_mutation() {
        ...
        assert!(ExportContext::at_expected(&path, home.clone(), Some("expected-account")).is_err());
        assert!(ExportContext::at_expected(&path, home.clone(), None)
            .unwrap()
            .account()
            .is_none());
        ...
        let context = ExportContext::at_expected(&path, home.clone(), Some(&runtime.id)).unwrap();
        assert!(context.runtime().unwrap().same_account(&runtime).unwrap());
        drop(context);
        assert!(ExportContext::at_expected(&path, home.clone(), Some("another-account")).is_err());
        let mut changed = config;
        changed.keys_file = root.path().join("other-keys.json");
        fs::write(&path, serde_json::to_vec(&changed).unwrap()).unwrap();
        assert!(ExportContext::at_expected(&path, home.clone(), Some(&runtime.id)).is_err());
        fs::remove_file(&path).unwrap();
        assert!(ExportContext::at_expected(&path, home, Some(&runtime.id)).is_err());
    }
    ...
    fn missing_configuration_and_ancestors_must_remain_missing() {
        ...
        let context = ExportContext::at(&path, root.path().join("runtime")).unwrap();
        ...
        assert!(context.verify().is_err());
        drop(context);
        assert!(ExportContext::at(&path, root.path().join("runtime")).is_err());
    }
    ...
    fn offline_protection_includes_the_future_config_and_input() {
        ...
        for target in [&path, &input] {
            assert!(crate::toolkit::ExportTarget::capture_paths(target, &protected).is_err());
        }
        assert!(!path.exists());
