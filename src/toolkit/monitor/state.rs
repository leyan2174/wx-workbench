...
use std::{collections::HashMap, fs::{self, OpenOptions}, io::Read, path::{Path, PathBuf}};
...
impl StateFile {
    pub fn new(context: &FixedRuntimeContext, sessions: HashMap<String, i64>) -> Self {
        Self { version:1, runtime_id:context.runtime_id().into(), sessions }
    }
}

fn unique_sessions<'de, D: Deserializer<'de>>(deserializer: D) -> std::result::Result<HashMap<String, i64>, D::Error> {
    struct SessionsVisitor;
    impl<'de> Visitor<'de> for SessionsVisitor {
        type Value = HashMap<String, i64>;
        fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { formatter.write_str("唯一会话 ID 与整数时间戳") }
...
pub(super) fn validate_entry(user: &str, timestamp: i64, latest: i64) -> Result<()> {
    ensure!(!user.trim().is_empty() && user.len() <= 512 && !user.chars().any(char::is_control), "游标会话 ID 无效");
    ensure!((0..=latest).contains(&timestamp), "游标时间戳必须非负且不得远在未来");
    Ok(())
}
...
pub(super) fn validate_size(sessions: &HashMap<String, i64>) -> Result<()> {
    ensure!(serde_json::to_vec(sessions)?.len() as u64 <= MAX_STATE_BYTES, "游标编码超过 8 MiB 上限");
    Ok(())
}
...
pub(super) fn load(context: &FixedRuntimeContext, path: &Path) -> Result<StateFile> {
    ...
    ensure!(meta.is_file() && !is_reparse(&meta) && meta.len() <= MAX_STATE_BYTES, "状态文件必须为不超过 8 MiB 的普通文件");
    let canonical = fs::canonicalize(&absolute)?;
    // 显式状态文件也不能被误设为当前账号的配置、密钥、数据库或后台缓存。
    for root in [&context.runtime.config.db_dir, &context.runtime.config.decrypted_dir, &context.runtime.cache_dir()] {
        if let Ok(root) = root.canonicalize() { ensure!(!canonical.starts_with(root), "状态文件不得位于数据库或缓存目录内"); }
    }
    for protected in [&context.runtime.config_path, &context.runtime.config.keys_file, &context.runtime.pid_path(), &context.runtime.log_path()] {
        if protected.try_exists()? {
            // 只比较文件身份，不读取这些受保护文件的内容；同时挡住硬链接别名。
            ensure!(!same_file::is_same_file(&canonical, protected)?, "状态文件指向受保护的账号文件");
        }
    }
    let mut options = OpenOptions::new(); options.read(true);
    #[cfg(windows)]
    { use std::os::windows::fs::OpenOptionsExt; options.share_mode(1); }
    let file = options.open(&canonical).context("无法以只读方式打开状态文件")?;
    let handle = same_file::Handle::from_file(file.try_clone()?)?;
    ensure!(handle == same_file::Handle::from_path(&canonical)?, "状态文件读取前被替换");
    let mut bytes = Vec::new();
    file.take(MAX_STATE_BYTES + 1).read_to_end(&mut bytes)?;
    ensure!(bytes.len() as u64 <= MAX_STATE_BYTES, "状态文件超过 8 MiB");
    let bytes = bytes.strip_prefix(b"\xef\xbb\xbf").unwrap_or(&bytes);
    let state: StateFile = serde_json::from_slice(bytes).map_err(|_| anyhow::anyhow!("状态文件 JSON/版本结构无效，不支持未绑定账号的旧格式"))?;
    ensure!(state.version == 1 && state.runtime_id == context.runtime_id(), "状态文件版本不支持或不属于当前账号");
    let latest = chrono::Utc::now().timestamp().saturating_add(86400);
    for (user, timestamp) in &state.sessions { validate_entry(user, *timestamp, latest)?; }
    Ok(state)
}
...
pub(super) fn checked_parent(path: &Path) -> Result<PathBuf> {
    ...
    for ancestor in parent.ancestors() {
        ...
        ensure!(metadata.is_dir() && !is_reparse(&metadata), "路径不得经过符号链接或重解析点");
    }
