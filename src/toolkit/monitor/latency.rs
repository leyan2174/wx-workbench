...
use crate::daemon::cache::{LatencyProbe, LATENCY_PROBE_KIND};
...
pub async fn run_latency(
    ...
    let mut observed: Option<SourceStamps> = None;
    ...
    loop {
        ...
        let stat_result = if options.ipc_only {
            None
        } else {
            Some(source_stamps(context))
        };
        let stat_ms = (!options.ipc_only).then(|| millis(stat_started.elapsed()));
        let current = match stat_result {
            None => None,
            Some(result) => match result {
                Ok(stamps) => {
                    if observed.as_ref().is_some_and(|old| old != &stamps) {
                        ...
                        sink(event(
                            ...
                            "file_change",
                            json!({
                                "database_changed":observed.as_ref().is_some_and(|old| old.database != stamps.database),
                                "wal_changed":observed.as_ref().is_some_and(|old| old.wal != stamps.wal),
                                "observed_poll_gap_ms":poll_gap_ms, "source":stamps
                            }),
                        ))?;
                    }
                    observed = Some(stamps.clone());
                    Some(stamps)
                }
                Err(error) => {
        ...
        if due || change_due {
            ...
            let mut candidate_sessions = sessions_seen.clone();
            let mut session_advances = 0usize;
            ...
            if failure.is_none() {
                ...
                {
                    Ok((sample, data)) => {
                        for row in data["sessions"].as_array().expect("已检查结果数组") {
                            let (Some(user), Some(timestamp)) =
                                (row["username"].as_str(), row["timestamp"].as_i64())
                            else {
                                failure = Some(transport::QueryFailure::protocol(
                                    "会话身份/时间无效".into(),
                                ));
                                break;
                            };
                            if state::validate_entry(
                                user,
                                timestamp,
                            ...
                            if sessions_seen.get(user).is_some_and(|old| timestamp > *old) {
                                session_advances += 1;
                            }
                            candidate_sessions
                                .entry(user.into())
                        ...
                        if candidate_sessions.len() > super::MAX_SESSIONS {
                            failure =
                                Some(transport::QueryFailure::protocol("会话数量超过上限".into()));
                        }
                        if failure.is_none() {
            ...
            if let Some(error) = failure {
                ...
                sink(event(
                    ...
                    json!({
                        ...
                        "probe_total_ms":millis(probe_started.elapsed()), "file_observation_to_probe_completion_ms":pending_since.map(|time| millis(time.elapsed())),
                        "session_timestamp_advances":session_advances, "session_history_timestamp_gap_seconds":gap,
                ...
                if !source_incomplete {
                    sessions_seen = candidate_sessions;
    ...
    summary.pending_file_change = pending_since.is_some();
