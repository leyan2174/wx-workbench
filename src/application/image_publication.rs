...
use crate::application::publication_context::PublicationContext as ImageScope;
#[cfg(test)]
use crate::attachment::decoder::{restore, V2KeyMaterial, V2_MAGIC};
...
pub(crate) fn decode_image_for(
    runtime: &RuntimeContext,
    input: String,
    output: Option<String>,
    stored: StoredImageKeys,
) -> Result<()> {
    decode_image_scoped(&ImageScope::for_runtime(runtime)?, input, output, stored)
}

fn decode_image_scoped(
    scope: &ImageScope,
    input: String,
    output: Option<String>,
    stored: StoredImageKeys,
...
pub(crate) fn batch_images_for(
    runtime: &RuntimeContext,
    input: String,
    output: Option<String>,
    stored: StoredImageKeys,
) -> Result<()> {
    batch_images_scoped(&ImageScope::for_runtime(runtime)?, input, output, stored)
}

fn batch_images_scoped(
    scope: &ImageScope,
    input: String,
    output: Option<String>,
    stored: StoredImageKeys,
...
mod workflow_tests {
    ...
    fn album_layout_collapses_thumbnail_suffix_and_is_idempotent() {
        ...
        let cipher: Vec<_> = plain.iter().map(|byte| byte ^ 0x55).collect();
