...
use super::export::{prepare_export, safe_dirname, write_export_with_media, ExportData, Timeline};
use super::*;
...
fn read_database(
    ...
    let snapshot = crate::adapters::wechat::moments::read_export_snapshot(
        sns,
        contacts,
        &options.contacts,
        options.timezone,
    )?;
    prepare_export(snapshot, options)
}
...
fn filtering_and_missing_interactions_are_explicit() {
    ...
    assert!(data
        .warnings
        .iter()
        .any(|s| s.contains("moments interactions")));
    ...
    sns.execute_batch("CREATE TABLE SnsMessage_tmp3(feed_id INTEGER);")
        .unwrap();
    assert!(crate::adapters::wechat::moments::export_comments(&sns, zone()).is_err());
}
