...
#[test]
#[cfg(windows)]
fn encrypted_invalid_or_empty_material_is_not_published() {
    ...
    // .NET AES-CBC, Padding=None, Key=IV=00..0f, plaintext=[16; 16].
    let all_padding = vec![
        0x07, 0xfe, 0xef, 0x74, 0xe1, 0xd5, 0x03, 0x6e, 0x90, 0x0e, 0xee, 0x11, 0x8e, 0x94, 0x92,
        0x93,
    ];
    for (ciphertext, stage, failure) in [
        (b"bad".to_vec(), Stage::Decode, Failure::InvalidMaterial),
        (Vec::new(), Stage::Download, Failure::Unavailable),
        (all_padding, Stage::Download, Failure::Unavailable),
    ] {
        ...
        .unwrap();
        let (url, server) = serve(vec![response(&ciphertext)]);
        conn.execute(
            "INSERT INTO kNonStoreEmoticonTable VALUES(?1,?2,'',?3,'')",
            [MD5, KEY, url.as_str()],
        )
        .unwrap();
