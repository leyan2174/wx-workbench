use super::super::cache::StoreStatus;
use super::*;
use crate::infrastructure::transcription::{local::LocalConfig, openai::OpenAiConfig};
...
fn local(dir: &Path) -> Backend {
    ...
    Backend::WhisperCpp(LocalConfig::new(program, model))
}
...
fn current_local_config_changes_miss_but_timeout_changes_hit() {
    ...
    let Backend::WhisperCpp(original) = backend else {
        panic!()
    };
    for field in 0..4 {
        ...
        match field {
            ...
            2 => {
                changed.output_format =
                    crate::infrastructure::transcription::local::OutputFormat::Json
            }
            _ => {
        ...
        assert!(matches!(
            find(&path, &Backend::WhisperCpp(changed)),
            LookupOutcome::Miss
        ));
    }
    let mut changed = original.clone();
    ...
    for input in [&original.executable, &original.model] {
        ...
        assert!(matches!(
            find(&path, &Backend::WhisperCpp(original.clone())),
            LookupOutcome::Miss
        ));
    ...
    assert!(matches!(
        find(&path, &Backend::WhisperCpp(original)),
        LookupOutcome::Unavailable
    ));
}
...
fn multiple_configs_share_proof_but_every_strong_evidence_change_marks_conflict() {
    for field in 0..10 {
        ...
        let Backend::WhisperCpp(config) = &backend else {
            panic!()
        };
        let mut second = config.clone();
        second.language = "en".into();
        let second = Backend::WhisperCpp(second);
...
fn real_cloud_authorization_precedes_io_and_current_identity_does_not_upload() {
    ...
    let Backend::OpenAiCompatible { allow_upload, .. } = &mut backend else {
        panic!()
    };
    *allow_upload = false;
    assert!(matches!(find(&path, &backend), LookupOutcome::Unavailable));
...
fn real_identity_holds_model_and_program_handles_through_atomic_publication() {
    ...
    let Backend::WhisperCpp(local) = &backend else {
        panic!()
    };
    Cache::open(&path, req.account)
        .unwrap()
