...
fn program_content_change_invalidates_cache() {
    ...
    let Backend::WhisperCpp(config) = &mut backend else {
        panic!()
    };
...
fn cloud_upload_hit_identity_isolation_and_authorization() {
    ...
    let make = |endpoint: &str, model: &str, language: Option<&str>, key: &str, limit| {
        Backend::openai_compatible(
            crate::infrastructure::transcription::openai::OpenAiConfig {
    ...
    let limit = crate::infrastructure::transcription::openai::OPENAI_AUDIO_LIMIT_BYTES;
...
fn oversized_input_is_rejected_before_container_or_model_access() {
    let dir = tempfile::tempdir().unwrap();
    let backend = Backend::WhisperCpp(
        crate::infrastructure::transcription::local::LocalConfig::new(
...
fn operational_timeout_does_not_invalidate_completed_local_transcription() {
    ...
    for timeout in [
        ...
        if let Backend::WhisperCpp(config) = &mut backend {
