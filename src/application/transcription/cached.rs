...
use crate::adapters::wechat::media::voice::VoiceEvidence;
...
fn transcribe(
    ...
    crate::infrastructure::audio::normalize_silk(request.silk)?;
...
pub(super) fn backend_name(backend: &Backend) -> &'static str {
    match backend {
        Backend::WhisperCpp(_) => "whisper_cpp",
        Backend::PythonWhisper(_) => "python_whisper",
        Backend::OpenAiCompatible { .. } => "openai_compatible",
    }
}
...
pub(super) fn identity(
    ...
    match backend {
        Backend::PythonWhisper(config) => {
            ...
            Ok((
                ConfigIdentity::new("python_whisper", &engine, "engine-defined", &options)?,
                Vec::new(),
                None,
            ))
        }
        Backend::WhisperCpp(config) => {
            ...
            let (program, program_file) =
                crate::infrastructure::transcription::file_digest(&config.executable)?;
            let (model, model_file) =
                crate::infrastructure::transcription::file_digest(&config.model)?;
            ...
            Ok((
                ...
                Some(Backend::WhisperCpp(config)),
            ))
        }
        Backend::OpenAiCompatible { client, .. } => {
            ...
            Ok((
                ConfigIdentity::new("openai_compatible", model, &language, &options)?,
                Vec::new(),
                None,
            ))
        }
