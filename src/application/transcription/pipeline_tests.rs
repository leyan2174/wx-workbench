...
fn native_silk_fixture_decodes_to_wav() {
    ...
    assert_eq!(
        &wav[44..],
        crate::infrastructure::audio::decode_silk_to_pcm(silk).unwrap()
    );
}
...
fn upload_denied_before_input_or_key_access() {
    ...
    assert!(Backend::openai_compatible(config, false)
        .unwrap_err()
        .to_string()
...
fn local_pipeline_and_chat_writeback_end_to_end() {
    ...
    let backend = Backend::WhisperCpp(config);
...
fn explicit_cloud_pipeline_uploads_wav_to_loopback_only() {
    ...
    let backend = Backend::openai_compatible(
        openai::OpenAiConfig {
            base_url: url,
