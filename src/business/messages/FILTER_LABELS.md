...
Sticker, Location and ContactCard are narrow independent labels because Kind
does not classify them. Link and File remain separate label intents, not
aliases for the semantic Structured kind.

`service/message_filter.rs` owns transport spelling profiles and the legacy
numeric projection. CLI remains lowercase-only with its original ten labels.
MCP trims whitespace, folds ASCII case, accepts namecard/app/emoji/voip and
still rejects the CLI-only link/sticker/call spellings. File and app deduplicate
to one legacy selector, as before. CLI value parsers are unchanged.
...
Tests cover canonical business labels, every legacy CLI/MCP mapping, alias and
case differences, real MCP routing, and real synthetic SQLite selection for
