...
use crate::{
    ...
    application::moments::archive::{self, ArchiveOptions, ArchiveReport},
...
pub fn cmd(args: Args) -> Result<()> {
    ...
    let keys = material
        .as_ref()
        .map_or_else(CacheKeys::default, |material| CacheKeys {
            image_aes_key: Some(material.aes),
            image_xor_key: material.xor,
        });
    crate::service::worker_keys::verify_image_revision(&runtime)?;
    let report = export_for(&runtime, &raw, args, &keys)?;
    crate::service::worker_keys::verify_image_revision(&runtime)?;
    println!(
        "{}",
