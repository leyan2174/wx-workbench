...
impl Pending {
    ...
    /// 按账号、消息及后端身份查找转录缓存；命中时不读取源语音或重复识别。
    /// 核对后端身份可能启动 Python worker；返回前仍检查运行身份、取消和授权。
    pub fn try_cached(
        &mut self,
        exact_username: &str,
        context: &CallContext,
...
fn configured_local_backend(
    ...
    let build = || -> anyhow::Result<asr::Backend> {
        ...
        let local = asr::local_python::LocalPythonConfig::discover(
            ...
            args.temp_root
                .clone()
                .ok_or_else(|| anyhow::anyhow!("private work directory missing"))?,
            runtime
                .config_path
                .parent()
...
mod configured_local_tests {
    ...
    fn configured_local_requires_host_opt_in_and_rejects_mixed_parameters() {
        ...
        assert!(matches!(
            Args::default().prepare(Operation::Transcribe, 1, None, &context),
        ...
        let candidates = [
            ...
            BackendArgs {
                api_key_file: Some("missing-private-key".into()),
                ..Default::default()
            },
            BackendArgs {
            ...
            BackendArgs {
                openai_model: Some(PRIVATE.into()),
                ..Default::default()
            },
            BackendArgs {
                whisper_binary: Some("missing-python".into()),
                ..Default::default()
            },
            BackendArgs {
                whisper_model: Some(PRIVATE.into()),
                ..Default::default()
            },
            BackendArgs {
                temp_root: Some("caller-selected-work".into()),
                ..Default::default()
            },
            BackendArgs {
                threads: Some(0),
        ...
        for backend in candidates {
            ...
            assert!(matches!(
                args.prepare_configured_local(1, &context),
                Err(DispatchError::Unavailable)
            ));
        }
    }
    ...
    fn configured_local_configuration_errors_are_redacted_and_bounded() {
        ...
        assert_eq!(
            configured_local_backend(&rt, &args).unwrap_err(),
    ...
    fn configured_local_account_switch_is_rejected_before_inference() {
        ...
        assert!(matches!(
            pending.finish(Response::ok(json!({})), &changed, &context, || Ok(())),
            Err(DispatchError::Unavailable)
        ));
        assert!(!work.exists());
    }
    ...
    fn configured_local_expired_call_is_rejected_before_preparation() {
        ...
        assert!(matches!(
            Args::default().prepare_configured_local(1, &context),
